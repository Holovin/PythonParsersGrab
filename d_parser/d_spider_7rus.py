        ...
        try:
            ...
            # ALTERNATIVE MODE
                ...
                if not product_info_sale.text(''):
                    self.log.warning(task, f'Unknown sale block, skip...')
                    return
                ...
                for row in table:
                    ...
                    if not row_product_count or not Ree.number.match(row_product_count):
                        self.log.warning(task, 'Empty row product, skip line...')
                        continue
            ...
            # D = unit (measure)
            product_unit = product_info.select('.//div[@id="technical_chars"]//tr[5]/td[2]/span').text('ед.')

            # F = vendor code (sku)
            product_vendor_code = product_info.select('.//div[@id="technical_chars"]//tr[5]/td[2]/span').text('')

            # G = vendor (manufacture)
            product_vendor = product_info.select('.//div[@id="technical_chars"]//tr[3]/td[2]/span/a').text('').rsplit('.', maxsplit=1)[0]
